"""Residual mode uncertainty on labelled text corpora.
...
The exact synthetic studies verify them where every constant is predicted. This
study's job is narrower: show that the *shape* both assumptions require occurs
...
__all__ = [
    ...
    "run_corpus_mode_uncertainty_corpus",
    "V_SMALL",
    "V_BRIDGE",
    "V_LARGE",
    "V_GRID",
...
CORPORA: dict[str, tuple[Side, Side]] = {
    ...
    # the regime boundary and this study needs a clean binary Z.
    "code_prose": (
...
def _cache_path(corpus: str, side: str, tok: str, n_docs: int, doc_tokens: int) -> Path:
    ...
    return CACHE_DIR / "corpus_mode_uncertainty" / f"{corpus}_{side}_{h}.npy"


def _legacy_cache_path(
    corpus: str, side: str, tok: str, n_docs: int, doc_tokens: int
) -> Path:
    """Read-only migration fallback for caches made by the former filename."""
    key = f"{corpus}|{side}|{tok}|{n_docs}|{doc_tokens}"
    h = hashlib.sha256(key.encode()).hexdigest()[:16]
...
def build_corpus(name: str, tokenizer_spec: str, n_docs: int, doc_tokens: int,
    ...
    for tag, side in (("plus", plus_side), ("minus", minus_side)):
        ...
        legacy_cache = _legacy_cache_path(name, tag, tok.name, n_docs, doc_tokens)
        readable_cache = cache if cache.exists() else legacy_cache
        if use_cache and readable_cache.exists():
            packed[tag] = np.load(readable_cache)
            if verbose:
                print(f"    {tag:5s}: {readable_cache.name} (cached)")
            continue
        t0 = time.time()
...
def build_markov_corpus(n_docs: int = 8000, doc_tokens: int = 1152,
    ...
    Kept so the corpus pipeline stays runnable and testable without network
...
def run_corpus_mode_uncertainty_corpus(
    cfg: RunConfig,
    corpus_name: str,
    tokenizer: str,
    n_docs: int,
...
def run_corpus_mode_uncertainty(
    cfg: RunConfig,
    ...
    for name in corpora:
        rd, fits, stats, cal, form = run_corpus_mode_uncertainty_corpus(
    ...
    write_table(realdata, "corpus_uncertainty", cfg, results_dir)
    write_table(fits, "corpus_uncertainty_fits", cfg, results_dir)
    write_table(stats, "corpus_uncertainty_stats", cfg, results_dir)
    write_table(calib, "corpus_uncertainty_calibration", cfg, results_dir)
    write_table(form, "corpus_uncertainty_form", cfg, results_dir)

    if verbose:
        print(f"  corpus mode uncertainty done in {time.time()-t0:.1f}s")
