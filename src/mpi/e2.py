"""E2 -- Low-visibility intervention.

Population-exact verification of the recovery scaling (Cor. `recovery`) and of
the full-mask boundary (Prop. `full-mask`).  E2 adds *no* enumeration cost: for
a two-point schedule ``mu = (1-pi) delta_m + pi delta_s`` every quantity is an
exact convex combination of the two component quantities already computed in E1
(spec section 2.5).
...
def run_e2(cfg: RunConfig, results_dir: Path | None = None, verbose: bool = True):
    t0 = time.time()
    ...
    for spec in cfg.law_specs():
        for w in cfg.w_grid:
            for N in cfg.N_grid:
                ...
                if not spec.pinned:
                    continue  # E2 is stated for pinned laws only
                tic = time.time()
                boost_rows.extend(_boost_rows(spec, N, w, cfg))
                if verbose:
                    print(f"  E2 {spec.key:>12s}  N={N:<5d} w={w:<4g}"
    ...
    if verbose:
        print(f"  E2 done in {time.time()-t0:.1f}s ({len(boost)} boost cells)")
