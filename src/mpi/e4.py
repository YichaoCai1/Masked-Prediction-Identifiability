"""E4 -- the sampling cost of low-visibility mass.

E1 and E2 are population statements: they evaluate ``D_mu`` exactly, as though
an infinite sample were available.  E4 asks the finite-sample question that
Prop. `full-mask` raises but does not answer -- *how many masked examples does
it take to see* a discrepancy that E1/E2 certify is nonzero?  This is the
...
``logsumexp`` over the same ``r`` grid that E1 already builds.
...
def _base_sizes(N: int) -> tuple[int, ...]:
    """Same two base visible sizes E2 uses, so the two tables line up."""
    return (N // 4, N // 2)
...
def run_e4(cfg: RunConfig, results_dir: Path | None = None, verbose: bool = True):
    """Compute the ``sampling`` table over the E2 grid."""
    t0 = time.time()
    rows: list[dict] = []
