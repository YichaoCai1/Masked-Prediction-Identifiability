"""Exact mode-blindness and sensitivity calculations.
...
def _numerical_ci(
    ...
    uncertainty for the exact analyses (spec section 3.1).  Leg (c), the float64-vs-mpmath
...
def run_mode_blindness(
    ...
    for spec in cfg.law_specs():
        for w in cfg.w_grid:
            for N in cfg.N_grid:
                ...
                if verbose:
                    print(
                        f"  mode blindness {spec.key:>12s}  N={N:<5d} w={w:<4g}"
    ...
    if verbose:
        print(f"  mode blindness done in {time.time()-t0:.1f}s "
